...
export function merge(target: Array<ShortformExportType>, from: Array<ShortformExportType>) {
    ...
    from.forEach(item => {
        if(sfs.indexOf(item.sf) != -1) {
            const existsInTarget = target.filter(existing => { return existing.sf == item.sf})[0]
